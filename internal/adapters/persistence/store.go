...
CREATE TABLE IF NOT EXISTS secrets (
  ref TEXT PRIMARY KEY, owner_id TEXT NOT NULL, secret_type TEXT NOT NULL,
  label TEXT, envelope TEXT NOT NULL, version INTEGER NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS system_incidents (
  ...
  resolved INTEGER NOT NULL DEFAULT 0, resolved_at INTEGER, resolved_by TEXT,
...
func (s *Store) PurgeDeletedUserMailByEmail(ctx context.Context, email string) ([]string, error) {
	...
	defer tx.Rollback()
	rows, err := tx.QueryContext(ctx, `SELECT DISTINCT a.user_id FROM mail_accounts a
		JOIN users u ON u.id=a.user_id WHERE lower(a.email)=lower(?) AND u.status='deleted'`, strings.TrimSpace(email))
	if err != nil {
		return nil, err
	}
	var userIDs []string
	...
	for _, userID := range userIDs {
		rawRows, err := tx.QueryContext(ctx, `SELECT raw_uri FROM messages WHERE user_id=? AND raw_uri<>''`, userID)
		if err != nil {
			return nil, err
		}
		for rawRows.Next() {
		...
		rawRows.Close()
		attRows, err := tx.QueryContext(ctx, `SELECT a.storage_uri FROM attachments a
			JOIN messages m ON m.id=a.message_id WHERE m.user_id=? AND a.storage_uri<>''`, userID)
		if err != nil {
			return nil, err
		}
		for attRows.Next() {
		...
		attRows.Close()
		stmts := []string{
			`DELETE FROM message_notes WHERE user_id=?`, `DELETE FROM message_collab WHERE user_id=?`,
			`DELETE FROM action_cards WHERE user_id=?`, `DELETE FROM embeddings WHERE user_id=?`,
			`DELETE FROM message_bodies WHERE message_id IN (SELECT id FROM messages WHERE user_id=?)`,
...
func (s *Store) PutSecretEnvelope(ctx context.Context, sec domain.StoredSecret) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO secrets
	 ...
	 ON CONFLICT(ref) DO UPDATE SET envelope=excluded.envelope, version=excluded.version,
	   label=excluded.label, revoked=0, updated_at=excluded.updated_at`,
...
func (s *Store) GetSecretEnvelope(ctx context.Context, ref string) (domain.StoredSecret, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT ref,owner_id,secret_type,label,envelope,version,revoked FROM secrets WHERE ref=?`, ref).
		Scan(&out.Ref, &out.Owner, &st, &out.Label, &out.Envelope, &out.Version, &revoked)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) MarkSecretEnvelopeRevoked(ctx context.Context, ref string) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE secrets SET revoked=1, envelope='', updated_at=? WHERE ref=?`, now(), ref)
	return err
}

func (s *Store) ListSecretEnvelopes(ctx context.Context) ([]domain.StoredSecret, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT ref,owner_id,secret_type,label,envelope,version,revoked FROM secrets WHERE revoked=0`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var st string
		var revoked int
		if err := rows.Scan(&sec.Ref, &sec.Owner, &st, &sec.Label, &sec.Envelope, &sec.Version, &revoked); err != nil {
			return nil, err
		}
		sec.Type = domain.SecretType(st)
		sec.Revoked = revoked != 0
		out = append(out, sec)
	}
	return out, rows.Err()
}
...
func (s *Store) RecordIncident(ctx context.Context, inc *domain.Incident) error {
	res, err := s.db.ExecContext(ctx,
		`UPDATE system_incidents SET count=count+1, last_seen=?, detail=?, severity=?, message=?
...
func (s *Store) GetIncident(ctx context.Context, id string) (*domain.Incident, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id,fingerprint,severity,component,message,COALESCE(detail,''),count,first_seen,last_seen,
		 resolved,COALESCE(resolved_at,0),COALESCE(resolved_by,''),COALESCE(account_id,''),COALESCE(job_id,'')
		 FROM system_incidents WHERE id=?`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) ResolveIncident(ctx context.Context, id, resolvedBy string) error {
	res, err := s.db.ExecContext(ctx,
		`UPDATE system_incidents SET resolved=1, resolved_at=?, resolved_by=? WHERE id=? AND resolved=0`,
		now(), resolvedBy, id)
	if err != nil {
		return err
	}
...
func (s *Store) IncidentStats(ctx context.Context) (domain.IncidentStats, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT
	 COALESCE(SUM(CASE WHEN resolved=0 AND severity='critical' THEN 1 ELSE 0 END),0),
	 COALESCE(SUM(CASE WHEN resolved=0 AND severity='error' THEN 1 ELSE 0 END),0),
	 COALESCE(SUM(CASE WHEN resolved=0 AND severity='warning' THEN 1 ELSE 0 END),0),
...
func scanIncidents(rows *sql.Rows) ([]domain.Incident, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&inc.ID, &inc.Fingerprint, &sev, &inc.Component, &inc.Message, &inc.Detail,
			&inc.Count, &inc.FirstSeen, &inc.LastSeen, &resolved, &inc.ResolvedAt, &inc.ResolvedBy,
...
func (s *Store) Search(ctx context.Context, q domain.SearchQuery) (*domain.SearchResult, error) {
	...
	if q.ReceivedSince > 0 {
		conds, args = append(conds, "m.created_at >= ?"), append(args, q.ReceivedSince)
	}
	if q.Until > 0 {
...
// worked on. A job is spared when (a) its ID is in activeJobIDs (running on
// this node) or (b) graceSeconds > 0 and its updated_at is within the grace
// window (a live worker on any node is still heart-beating it). This is what
// keeps a leader-election flap or failover from killing an in-flight sync.
func (s *Store) RecoverStaleJobsExcept(ctx context.Context, activeJobIDs []string, graceSeconds int) (int64, error) {
	...
	query := "UPDATE jobs SET status='failed', error='interrupted (worker restart or failover)', updated_at=? WHERE " + where
...
func (s *Store) CreateMCPOAuthClient(ctx context.Context, c *domain.MCPOAuthClient) error {
	...
	_, err = s.db.ExecContext(ctx, `INSERT INTO mcp_oauth_clients (id, name, secret_hash, redirect_uris_json, auth_method, created_at, last_used_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)`, c.ID, c.Name, c.SecretHash, string(uris), c.AuthMethod, c.CreatedAt, c.LastUsedAt)
	return err
}

func scanMCPOAuthClient(row interface{ Scan(...any) error }) (*domain.MCPOAuthClient, error) {
...
func (s *Store) GetMCPOAuthClient(ctx context.Context, id string) (*domain.MCPOAuthClient, error) {
	return scanMCPOAuthClient(s.db.QueryRowContext(ctx, `SELECT id, name, secret_hash, redirect_uris_json, auth_method, created_at, last_used_at
		FROM mcp_oauth_clients WHERE id = ?`, id))
}

func (s *Store) ListMCPOAuthClients(ctx context.Context) ([]domain.MCPOAuthClient, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, secret_hash, redirect_uris_json, auth_method, created_at, last_used_at
...
func (s *Store) CountMCPOAuthClients(ctx context.Context) (int, error) {
	var n int
	err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM mcp_oauth_clients`).Scan(&n)
	return n, err
}

func (s *Store) TouchMCPOAuthClient(ctx context.Context, id string, lastUsedAt int64) error {
	_, err := s.db.ExecContext(ctx, `UPDATE mcp_oauth_clients SET last_used_at = ? WHERE id = ?`, lastUsedAt, id)
	return err
}

func (s *Store) DeleteMCPOAuthClient(ctx context.Context, id string) error {
	res, err := s.db.ExecContext(ctx, `DELETE FROM mcp_oauth_clients WHERE id = ?`, id)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (s *Store) PruneMCPOAuthClients(ctx context.Context, unusedBefore int64) (int64, error) {
	res, err := s.db.ExecContext(ctx, `DELETE FROM mcp_oauth_clients WHERE MAX(created_at, last_used_at) < ?`, unusedBefore)
	if err != nil {
		return 0, err
	}

...
MCP와 REST의 공개 오류는 `code`, `message`, 선택적인 `details`, `trace_id`를 공유합니다. 알 수 없는 DB·AI·메일 서버 오류 문자열은 그대로 노출하지 않습니다. REST의 기존 `error` 문자열은 이전 클라이언트 호환용으로 함께 유지할 수 있습니다. MCP 도구 오류는 `isError: true` 와 함께 오류 객체를 text content 로, 같은 객체를 `_meta.postra_error` 로 반환합니다. 도구가 공표한 출력 스키마는 성공 응답의 계약이고 오류 봉투는 그 형태를 만족할 수 없으므로, 오류 결과에는 `structuredContent` 를 싣지 않습니다. 이를 검증하는 클라이언트가 실패한 호출마다 스키마 오류를 보고하지 않게 하기 위함입니다. JSON-RPC 입력/리소스 오류는 프로토콜 오류일 수 있습니다.

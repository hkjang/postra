...
- 거절이 아닌 **진짜 실패**(잘못된 client secret, 허용되지 않은 scope·redirect_uri, 비활성 사용자,
  ...
  쌓이므로 그 자리를 떠나는 것입니다. 사유는 서버 로그에 남고, Discovery 실패(호스트 없음·연결 거부·
  시간 초과·TLS 인증서·HTTP 상태·issuer 불일치·JSON 아님으로 분류, issuer 표시), 제공자가 콜백에
  돌려준 오류(`invalid_scope` 등 허용된 코드만, `access_denied` 는 경고), 토큰 교환·검증 실패는
  장애 화면(`oidc` 구성 요소)에도 기록됩니다. 조용한 시도의 `login_required` 는 기록하지 않습니다.

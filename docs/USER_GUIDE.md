...
공식 React 화면은 `/app/`에서 사용할 수 있습니다. v0.20에서 이전 `/ui/` 템플릿을 제거했으며, 메일·업무·설정 기능과 Keycloak 이전 절차는 [React 워크스페이스 가이드](REACT_WORKSPACE.md)를 참고하세요. 아래 릴리즈 이력의 예전 URL은 당시 기능을 기록한 것입니다.
...
### 주요 비즈니스 및 기술적 이점
1. **멀티 계정 통합 워크스페이스**: 사내외 여러 POP3, IMAP, SMTP 계정을 단일 통합 웹 워크스페이스(`/app/`)에서 일관되게 관리합니다.
...
### 6.1 서식 메일 작성 및 초안 저장

- **[새 메일]**에서 수신자(`To`), 참조(`Cc`), 제목과 본문을 작성합니다. 기본 작성 방식인 **서식 편집**에서는 굵게, 기울임, 밑줄, 글자색, 제목, 목록, 정렬, 링크를 사용할 수 있습니다.
- **편지 / 공지 / 업무 공유** 디자인을 누르면 작성한 본문을 유지하면서 메일용 레이아웃을 적용합니다. 제목과 내용은 그대로 편집할 수 있습니다.
- 준비된 HTML이 있으면 작성 방식을 **HTML 직접 입력**으로 바꾸어 붙여 넣으세요. 일반 메일은 **일반 텍스트**를 선택합니다. 텍스트로 전환하여 저장하면 기존 HTML 서식은 제거됩니다.
- **초안 만들기** 또는 **변경 저장**을 눌러 저장한 뒤 **발송 미리보기·승인**에서 실제 서식을 확인합니다. 자동 저장은 아니므로 수정 후 반드시 저장하세요. 이후 **이 내용으로 승인 요청 → 발송 확정** 순서로 보냅니다.
- 서식 메일은 `multipart/alternative` 형식으로 HTML과 일반 텍스트를 함께 전송합니다. HTML을 표시하지 않는 수신 환경에서도 내용을 읽을 수 있습니다. 글꼴·여백 등은 수신 메일 프로그램에 따라 달라질 수 있습니다.
- 편집기는 서버에 포함되어 오프라인망에서도 외부 CDN 없이 동작합니다. 안전을 위해 스크립트, 폼, 외부 이미지 등은 제거하며, 기본적인 HTML 요소와 안전한 인라인 서식을 지원합니다.
- AI 작성 또는 **AI로 다시 쓰기**는 새 일반 텍스트 초안을 만듭니다. 생성된 내용에 서식을 다시 적용할 수 있습니다.

REST API의 초안 생성·수정에서도 `body_html`을 사용할 수 있습니다. HTML을 보내면 서버가 안전하게 정리한 뒤 일치하는 텍스트 본문을 생성하며, `body`만 수정하면 이전 HTML은 제거합니다.
...
---
*AI Infra실 — Postra 이메일 플랫폼 사용자 가이드 v0.20.0*

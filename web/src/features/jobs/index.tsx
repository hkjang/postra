...
type Job = {id: string; account_id?: string; type: string; status: string; progress?: string; error?: string; stats?: Record<string, number>; created_at: number}
...
export function JobsPage() {
  ...
  const jobs = useQuery({queryKey: ['jobs', id || 'list'], queryFn: async ({signal}): Promise<Job[]> => id ? [await api<Job>(`/api/jobs/${encodeURIComponent(id)}`, {signal})] : await api<Job[]>('/api/jobs?limit=100', {signal}), refetchInterval: query => query.state.data?.some(job => ['queued', 'running'].includes(job.status)) ? 2000 : false})

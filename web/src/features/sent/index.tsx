...
type Outbound = {id: string; draft_id: string; status: string; smtp_response?: string; attempts: number; created_at: number; next_attempt_at?: number}
type OutboundView = {outbound: Outbound; subject?: string; to?: string[]}
...
export function SentPage() {
  const out = useQuery({queryKey: ['outbound'], queryFn: ({signal}) => api<OutboundView[]>('/api/outbound?limit=100', {signal}), refetchInterval: query => query.state.data?.some(item => ['queued', 'sending', 'retry_wait'].includes(item.outbound.status)) ? 5000 : false})

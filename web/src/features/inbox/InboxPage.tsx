...
type MailPage = { messages: Message[]; next_cursor?: string; views?: Record<string, MessageView> }
type SearchMode = 'keyword' | 'semantic' | 'hybrid'
const filters = [{ id: 'inbox', label: '전체' }, { id: 'important', label: '중요' }, { id: 'attachment', label: '첨부' }, { id: 'today', label: '오늘' }, { id: 'archive', label: '보관함' }]
...
export function InboxPage() {
  ...
  const mode = (['semantic', 'hybrid'].includes(params.get('mode') || '') ? params.get('mode') : 'keyword') as SearchMode
  ...
  const mail = useInfiniteQuery({
    queryKey: ['messages', { query, mode, folder, account }],
    initialPageParam: '',
    queryFn: async ({ pageParam, signal }): Promise<MailPage> => {
      ...
      const search = new URLSearchParams({ q: query, limit: '60', cursor: pageParam, folder: ['important', 'archive'].includes(folder) ? folder : 'inbox' })
      if (account) search.set('account_id', account)
      if (folder === 'attachment') search.set('has_attachment', 'true')
      if (folder === 'today') { const today = new Date(); today.setHours(0, 0, 0, 0); search.set('since', String(Math.floor(today.getTime() / 1000))) }
  ...
  function select(message: Message) { change({ message: message.id }) }
  return <div className={`split-workspace mail-workspace ${selected ? 'has-selection' : ''}`}>
    <section className={`mail-list ${compact ? 'compact' : ''}`} aria-label="메일 목록">
      <header className="mail-list-header"><div className="row"><div><p className="eyebrow">YOUR WORKSPACE</p><h1>{query ? '검색 결과' : folder === 'important' ? '중요 메일' : folder === 'archive' ? '보관함' : '받은메일'}</h1></div><Button size="icon" variant="ghost" aria-label="메일 목록 새로고침" disabled={mail.isFetching} onClick={() => mail.refetch()}><RefreshCw size={18} className={mail.isFetching ? 'spin' : ''} /></Button></div>
        ...
        <div className="row mail-search-options"><select aria-label="검색 방식" value={mode} onChange={event => change({ mode: event.target.value, message: '' })}><option value="keyword">키워드 검색</option><option value="semantic">의미 검색 · AI</option><option value="hybrid">통합 검색 · AI</option></select><Button size="icon" variant="ghost" aria-label={compact ? '편안한 목록 밀도' : '촘촘한 목록 밀도'} aria-pressed={compact} onClick={() => setCompact(value => !value)}><ListFilter size={17} /></Button></div>
      ...
      {mail.isPending ? <Loading /> : mail.error ? <ErrorState error={mail.error} retry={() => mail.refetch()} /> : !messages.length ? <EmptyState title={query ? '검색 결과가 없습니다' : '아직 받은 메일이 없습니다'} description={query ? '검색어나 검색 방식을 바꿔 다시 찾아보세요.' : '계정에서 동기화 상태를 확인해 주세요. 수집된 메일이 이곳에 표시됩니다.'} /> : <div className="mail-virtual-list" onKeyDown={event => {
        ...
        event.preventDefault(); const current = messages.findIndex(message => message.id === selected); const next = event.key === 'j' ? Math.min(current + 1, messages.length - 1) : Math.max(current - 1, 0); select(messages[next])
      }}>
        <Virtuoso data={messages} computeItemKey={(_, message) => message.id} endReached={() => { if (mail.hasNextPage && !mail.isFetchingNextPage) mail.fetchNextPage() }} itemContent={(_, message) => <button className={`mail-list-item ${selected === message.id ? 'selected' : ''}`} onClick={() => select(message)} aria-current={selected === message.id ? 'true' : undefined}>
          ...
          <p className="mail-snippet">{views[message.id]?.body?.text_body?.slice(0, 160) || message.from.email}</p>
        ...
        </button>} components={{ Footer: () => mail.hasNextPage ? <div className="mail-list-footer"><Button variant="ghost" size="sm" disabled={mail.isFetchingNextPage} onClick={() => mail.fetchNextPage()}><ArrowDown size={15} />{mail.isFetchingNextPage ? '불러오는 중…' : '더 불러오기'}</Button></div> : <p className="mail-list-footer muted">{messages.length}개 메일 · 목록 끝</p> }} />
      </div>}
    </section>
    {selected ? <MessagePane key={selected} id={selected} onClose={() => change({ message: '' })} /> : <div className="mail-welcome"><div className="mail-welcome-icon"><Inbox size={36} strokeWidth={1.4} /></div><h2>메일에서 다음 할 일까지</h2><p>메일을 선택하면 본문과 AI Insight를<br />한곳에서 확인할 수 있습니다.</p><span className="muted">J / K 메일 이동 · 검색에서 의미까지 찾기</span></div>}
  </div>
}

...
function NavigationGuard({dirty, busy, onDiscard}: {dirty: boolean; busy: boolean; onDiscard: () => void}) {
  ...
  useEffect(() => {if (!dirty && !busy && blocker.state === 'blocked') blocker.reset()}, [dirty, busy, blocker])
  return <Dialog.Root open={blocker.state === 'blocked'} onOpenChange={open => {if (!open && blocker.state === 'blocked') blocker.reset()}}><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="settings-confirm"><Dialog.Title>{busy ? '설정을 저장하고 있습니다' : '저장하지 않은 설정이 있습니다'}</Dialog.Title><Dialog.Description>{busy ? '저장 결과를 확인한 뒤 이동해 주세요.' : '이동하면 입력 중인 값과 비밀값이 지워집니다. 설정을 저장하려면 이 화면에 머무르세요.'}</Dialog.Description><div className="row"><Button variant="outline" onClick={() => {if (blocker.state === 'blocked') blocker.reset()}}>계속 편집</Button><Button disabled={busy} onClick={() => {if (!busy && blocker.state === 'blocked') {onDiscard(); blocker.proceed()}}}>변경 버리고 이동</Button></div></Dialog.Content></Dialog.Portal></Dialog.Root>
}
...
function TaskModels({value, onChange, disabled}: {value: string; onChange: (value: string) => void; disabled: boolean}) {
  ...
  try {
    ...
    if (!object(parsed) || !Object.values(parsed).every(route => object(route) &&
      ['model', 'base_url', 'api_key_ref'].every(key => route[key] === undefined || typeof route[key] === 'string') &&
      (route.max_tokens === undefined || (typeof route.max_tokens === 'number' && Number.isSafeInteger(route.max_tokens) && route.max_tokens >= 0)))) invalid = true
    else routes = parsed as Record<string, Route>
  } catch { invalid = true }
  ...
  return <div className="stack">{invalid && <p role="alert">작업별 모델 JSON 형식을 확인해 주세요. 각 작업은 객체이고 모델·Endpoint·Secret 참조는 문자열, 토큰 한도는 0 이상의 정수여야 합니다. 기존 내용을 보존하기 위해 간편 입력을 잠시 잠급니다.</p>}<div className="task-model-grid">{tasks.map(([key, label]) => <label key={key} className="field"><span>{label}</span><Input disabled={disabled || invalid} value={routes[key]?.model || ''} placeholder="기본 Chat Model 상속" onChange={event => change(key, event.target.value)}/></label>)}</div><details open={invalid || undefined}><summary>전용 Endpoint·토큰 한도 고급 설정</summary><textarea aria-label="작업별 모델 JSON" aria-invalid={invalid} className="input" rows={6} value={value} disabled={disabled} onChange={event => onChange(event.target.value)}/><small className="muted">작업별 model, base_url, max_tokens, api_key_ref만 지정합니다. 비밀번호나 API Key 원문을 넣지 마세요. 임베딩은 전용 Embedding Model 항목을 사용합니다.</small></details></div>
}

export function SettingsEditor({admin = false, accountID, category, query: externalQuery, onPendingChange}: {admin?: boolean; accountID?: string; category?: string; query?: string; onPendingChange?: (pending: boolean) => void}) {
  ...
  const [secretReentry, setSecretReentry] = useState(false)
  ...
  useEffect(() => {onPendingChange?.(dirty > 0 || busy)}, [dirty, busy, onPendingChange])
  ...
  useEffect(() => {
    const leave = (event: BeforeUnloadEvent) => {if ((dirty || busy) && !logoutAllowed.current) {event.preventDefault(); event.returnValue = ''}}
    const logout = (event: Event) => {
      if (event.defaultPrevented) return
      if (busy || saving.current) {event.preventDefault(); toast.error('설정 저장이 끝난 뒤 로그아웃해 주세요.'); return}
      if (logoutAllowed.current) return
      if (dirty && !window.confirm('저장하지 않은 설정과 입력한 비밀값이 사라집니다. 로그아웃하시겠습니까?')) {event.preventDefault(); return}
      logoutAllowed.current = true; setLogoutPending(true)
    }
    const failed = () => {logoutAllowed.current = false; setLogoutPending(false)}
    window.addEventListener('beforeunload', leave)
    window.addEventListener('postra:before-logout', logout)
    window.addEventListener('postra:logout-failed', failed)
  ...
  const discard = () => {setValues({}); setSecrets({}); setLocks({}); setReset([]); setConfirm(false); setError(undefined); setSecretReentry(false); probe.clear()}
  ...
  function testCandidate(target: string) {
    if (busy || logoutAllowed.current) return
    setError(undefined)
  ...
  async function save() {
    if (!dirty || busy || saving.current || logoutAllowed.current) return
    saving.current = true
    probe.clear(); setBusy(true); setError(undefined); setSecretReentry(false)
    try {
  ...
  if (current.error && !current.data) return <ErrorState error={current.error} retry={() => current.refetch()}/>
  return <div className="settings-editor stack">
    {dataRouter && <NavigationGuard dirty={dirty > 0 && !logoutPending} busy={busy && !logoutPending} onDiscard={discard}/>}
    ...
    {secretReentry && <p role="status">저장 실패 후 입력한 비밀값은 보안을 위해 지웠습니다. API Key나 비밀번호를 변경하려면 다시 입력해 주세요.</p>}
    {current.error && <><ErrorState error={current.error} retry={() => current.refetch()}/><p className="muted">최신 설정을 가져오지 못했습니다. 입력한 변경사항은 보존되며, 저장 시 다른 운영자의 변경 여부를 확인합니다.</p></>}
    {admin && ['ai', 'search', 'auth', 'storage'].includes(category || '') && <div className="stack">
      {['ai', 'search'].includes(category || '') && <>
        <label className="field probe-task"><span>Chat 진단 작업</span><select className="input" aria-label="Chat 진단 작업" disabled={busy || logoutPending} value={probeTask} onChange={event => {probe.clear(); setProbeTask(event.target.value)}}><option value="">기본 Chat Model</option>{tasks.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
        <div className="row settings-probe-actions"><Button variant="outline" disabled={busy || logoutPending || probe.busy} onClick={() => testCandidate('ai_models')}>저장 전 Chat 모델 한도 조회</Button><Button variant="outline" disabled={busy || logoutPending || probe.busy} onClick={() => testCandidate('embedding_models')}>저장 전 Embedding 모델 한도 조회</Button></div>
        <small className="muted">모델 한도 조회는 /models 메타데이터만 읽으며 Chat 생성·임베딩을 실행하지 않습니다.</small>
      </>}
      <div className="row settings-probe-actions"><Button variant="outline" disabled={busy || logoutPending || probe.busy} onClick={() => testCandidate(category === 'auth' ? 'oidc' : category === 'storage' ? 'database' : 'ai')}>{category === 'auth' ? '저장 전 OIDC Discovery 확인' : category === 'storage' ? '현재 Database 연결 확인' : '저장 전 AI 연결 확인'}</Button>{['ai', 'search'].includes(category || '') && <Button variant="outline" disabled={busy || logoutPending || probe.busy} onClick={() => testCandidate('embedding')}>저장 전 Embedding 확인</Button>}</div>
    ...
    <div className="settings-fields">{selected.map(field => {
      ...
      const disabled = busy || logoutPending || ['deployment', 'fixed'].includes(field.apply) || (!admin && field.locked)
      ...
      return <section className="setting-row" key={field.key}>
        ...
        <div className="setting-control">{field.key === 'ai.task_models' ? <TaskModels value={value} onChange={value => edit(field, value)} disabled={disabled}/> : field.type === 'bool' ? <label className="row setting-toggle"><input id={id} type="checkbox" disabled={disabled} checked={value === 'true'} onChange={event => edit(field, String(event.target.checked))}/>{value === 'true' ? '사용' : '사용 안 함'}</label> : field.type === 'enum' ? <select className="input" id={id} disabled={disabled} value={value} onChange={event => edit(field, event.target.value)}>{field.options?.map(option => <option key={option} value={option}>{optionLabels[option] || option || '기본값'}</option>)}</select> : ['account', 'signature'].includes(field.type) ? <select id={id} className="input" disabled={disabled} value={value} onChange={event => edit(field, event.target.value)}><option value="">자동 선택 / 없음</option>{(field.type === 'account' ? accounts.data || [] : signatures.data || []).map(item => <option key={item.id} value={item.id}>{item.name || item.id}</option>)}</select> : field.type === 'json' || field.type === 'text' ? <textarea id={id} className="input" rows={4} disabled={disabled} value={value} onChange={event => edit(field, event.target.value)}/> : <Input id={id} type={field.secret ? 'password' : ['int', 'number'].includes(field.type) ? 'number' : field.type === 'url' ? 'url' : 'text'} step={field.type === 'number' ? 'any' : undefined} autoComplete={field.secret ? 'new-password' : 'off'} disabled={disabled} placeholder={field.secret ? secretLabel(field) : undefined} value={value} onChange={event => edit(field, event.target.value)}/>}
          ...
          {admin && field.lockable && <label className="row policy-lock"><input type="checkbox" checked={locks[field.key] ?? field.locked} disabled={busy || logoutPending} onChange={event => {probe.clear(); setLocks(old => {const next = {...old}; if (event.target.checked === field.locked) delete next[field.key]; else next[field.key] = event.target.checked; return next})}}/><LockKeyhole size={13}/>관리자 값 강제 적용</label>}
          {!admin && !field.locked && ['user', 'account'].includes(field.source) && <Button variant="ghost" size="sm" disabled={busy || logoutPending || reset.includes(field.key)} onClick={() => {setReset(old => [...old, field.key]); setValues(old => {const next = {...old}; delete next[field.key]; return next})}}>{reset.includes(field.key) ? '상속값으로 복원 예정' : '기본값 상속으로 복원'}</Button>}
          {field.type === 'signature' && <Link to="/settings/signatures">서명 여러 개 관리</Link>}
        </div>
      </section>
    })}</div>
    <footer className="settings-savebar"><span>{logoutPending ? '로그아웃 처리 중입니다.' : dirty ? `${changedFields.length}개 항목 변경 · 아직 저장되지 않음` : secretReentry ? '비밀값 저장이 완료되지 않았습니다. 다시 입력해 주세요.' : '모든 변경사항이 저장되었습니다.'}</span><div className="row">{dirty > 0 && <Button variant="ghost" disabled={busy || logoutPending} onClick={discard}>변경 취소</Button>}<Button disabled={!dirty || busy || logoutPending} onClick={() => setConfirm(true)}><Save size={15}/>변경 확인</Button></div></footer>
    <Dialog.Root open={confirm} onOpenChange={open => {if (!busy && !logoutPending) setConfirm(open)}}><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="settings-confirm"><div className="row between"><Dialog.Title>저장할 변경사항</Dialog.Title><Dialog.Close asChild><Button variant="ghost" size="icon" aria-label="닫기" disabled={busy || logoutPending}><X size={18}/></Button></Dialog.Close></div><Dialog.Description>조직 정책은 개인 설정보다 우선합니다. 재시작 항목은 기존 연결을 유지한 채 다음 실행에 적용됩니다.</Dialog.Description><ul>{changedFields.map(field => <li key={field.key}><strong>{field.label}</strong><p>{field.secret ? '비밀값 등록·교체 (원문 표시 안 함)' : reset.includes(field.key) ? '기본값 상속으로 복원' : `${field.value || '(비어 있음)'} → ${values[field.key] ?? field.value}`}</p>{field.key in locks && <p>{locks[field.key] ? '사용자 변경 금지' : '사용자 변경 허용'}</p>}{field.apply === 'restart' && <Badge variant="outline">재시작 필요</Badge>}</li>)}</ul>{error != null && <ErrorState error={error}/>} {secretReentry && <p>입력한 비밀값은 지워졌습니다. 닫은 뒤 다시 입력해 주세요.</p>}<Button disabled={busy || logoutPending || !dirty} onClick={save}>{busy ? '저장 중…' : '확인 후 저장'}</Button></Dialog.Content></Dialog.Portal></Dialog.Root>
  </div>
}

...
> **v0.10.2**: 크래시 증거 파이프라인 — fatal error 덤프 캡처, 비정상 종료(OOMKill/liveness kill) 자동 감지 후 Incident 기록, cgroup 기반 GOMEMLIMIT 자동 설정으로 OOM 완화.

| 파일 | 설명 |
| --- | --- |
| `postra-0.10.2-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.10.2`, linux/amd64) |
| `postra-0.10.2-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.10.2-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.10.2-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.10.2-linux-amd64
./postra-0.10.2-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.10.2-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.10.2

...
> **v0.16.11**: 배경 작업 쿼리 인덱스 추가 — 자동 트리아지(수 분 주기)와 일일 브리핑이 사용하는 (사용자, 수신시각) 조회에 인덱스가 없어 메일함이 커질수록 전체 스캔 부담이 누적되던 문제를 해결했습니다. 기존 데이터베이스는 기동 시 자동 생성됩니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.16.11-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.16.11`, linux/amd64) |
| `postra-0.16.11-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.16.11-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.16.11-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.16.11-linux-amd64
./postra-0.16.11-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.16.11-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.16.11

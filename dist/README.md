...
> **v0.16.5**: 백그라운드 워커 실패 처리 수정 — 특정 메일 하나로 자동 트리아지가 영구 중단되던 문제와 같은 메일을 매 주기 무한 재시도하며 AI 비용을 소모하던 문제 해결, AI가 잠시 불통일 때 하루치 브리핑이 통째로 사라지던 문제 수정(다음 주기에 재시도).

| 파일 | 설명 |
| --- | --- |
| `postra-0.16.5-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.16.5`, linux/amd64) |
| `postra-0.16.5-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.16.5-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.16.5-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.16.5-linux-amd64
./postra-0.16.5-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.16.5-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.16.5

...
> **v0.18.3**: 할 일 카드 화면 추가 — 메일에서 추출한 처리 항목(승인·회의·할 일)을 상단 `할 일` 메뉴에서 상태별로 보고 승인·완료·보류로 처리할 수 있습니다. 메일 화면에 카드 추출 버튼을 추가했으며, 기한·담당자는 메일 내용을 옮긴 값이므로 확인 후 처리하도록 안내합니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.18.3-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.18.3`, linux/amd64) |
| `postra-0.18.3-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.18.3-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.18.3-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.18.3-linux-amd64
./postra-0.18.3-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.18.3-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.18.3

...
> **v0.22.0 — Keycloak OAuth MCP 연결**: 기존 API 키를 유지하면서 OAuth 기반 MCP 인증과 관리자·사용자 연결 안내를 추가했습니다. OAuth는 기본 비활성화이며 사용하려면 별도 Keycloak MCP 클라이언트·PKCE·Audience 설정이 필요합니다. [릴리즈 내용](../docs/releases/v0.22.0.md)과 [설정 가이드](../docs/MCP_OAUTH.md)를 확인하세요. v0.19.x 이하에서 업그레이드할 경우 [v0.20.0의 백업·Keycloak 콜백·권한 변경 안내](../docs/releases/v0.20.0.md)도 먼저 확인하세요.

기존 SSO·메일 프로비저닝·비밀값 암호화·승인·멱등 발송은 유지합니다. 런타임 Node 서버나 외부 CDN은 필요 없으며 서체와 시간대 데이터도 실행 파일에 포함됩니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.22.0.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.22.0`, linux/amd64) |
| `postra-0.22.0-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.22.0-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
| `postra-0.22.0-frontend-sbom.cdx.json` | 프런트엔드 의존성 CycloneDX 명세서 |
...
docker load -i postra-0.22.0.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.22.0-linux-amd64
./postra-0.22.0-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-with-a-long-secret' ./postra-0.22.0-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.22.0

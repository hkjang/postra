...
> **v0.15.0**: AI 파이프라인 고도화 — 신규 메일 자동 시맨틱 인덱싱(백그라운드 임베딩 워커)으로 시맨틱·하이브리드 검색과 RAG 질의응답이 항상 최신 상태 유지, 메일 본문의 프롬프트 인젝션 시도 자동 탐지·감사·Incident 기록, AI 답변의 근거 메일 ID 검증(환각 인용 제거).

| 파일 | 설명 |
| --- | --- |
| `postra-0.15.0-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.15.0`, linux/amd64) |
| `postra-0.15.0-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.15.0-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.15.0-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.15.0-linux-amd64
./postra-0.15.0-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.15.0-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.15.0

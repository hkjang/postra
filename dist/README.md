...
> **v0.16.9**: 계정별 시맨틱 검색 누락 수정 — 여러 계정의 메일이 한 배치로 색인될 때 임베딩이 모두 첫 메일의 계정으로 저장되어, 계정을 지정한 시맨틱·하이브리드 검색과 RAG 답변에서 다른 계정 메일이 통째로 빠지던 문제를 수정했습니다. 계정이 둘 이상인 사용자는 재색인 없이 이후 색인부터 정상 반영됩니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.16.9-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.16.9`, linux/amd64) |
| `postra-0.16.9-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.16.9-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.16.9-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.16.9-linux-amd64
./postra-0.16.9-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.16.9-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.16.9

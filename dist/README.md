...
> **v0.16.10**: AI 작성 출처 표기 수정 — AI 빠른 답장 제안을 선택해 만든 초안이 "사용자 작성"으로 표시·기록되던 문제를 수정했습니다. 이제 모델이 쓴 본문은 "AI 작성"으로 표기되며 감사기록에도 동일하게 남습니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.16.10-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.16.10`, linux/amd64) |
| `postra-0.16.10-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.16.10-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.16.10-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.16.10-linux-amd64
./postra-0.16.10-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.16.10-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.16.10

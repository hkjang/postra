...
> **v0.3.0 새 기능**: 웹 UI(`http://<host>:8480/ui` — 검색·초안·발송 승인), IMAP 수집(`--inbound-protocol imap`), Prometheus 메트릭(`/metrics`), CI(govulncheck·gosec·SBOM).

| 파일 | 설명 |
| --- | --- |
| `postra-0.3.0-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.3.0`, linux/amd64) |
| `postra-0.3.0-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
...
docker load -i postra-0.3.0-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.3.0-linux-amd64
./postra-0.3.0-linux-amd64 init
POSTRA_ALLOW_INSECURE_MAIL=true ./postra-0.3.0-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.3.0

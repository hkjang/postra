...
> **v0.10.3**: Keycloak/OIDC 로그인 시 기존 로컬 계정(부트스트랩 어드민 포함)을 안전하게 연동 — (issuer,subject) 미일치 시 검증된 이메일 또는 사용자명 일치로 기존 계정에 연결하여 401/중복 계정 생성을 방지. OIDC 콜백 실패 사유 로깅 추가.

| 파일 | 설명 |
| --- | --- |
| `postra-0.10.3-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.10.3`, linux/amd64) |
| `postra-0.10.3-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.10.3-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.10.3-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.10.3-linux-amd64
./postra-0.10.3-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.10.3-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.10.3

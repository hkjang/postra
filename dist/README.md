...
> **v0.10.1**: 백그라운드 워커 및 Postgres 설정 리스너(LISTEN/NOTIFY)의 미복구 panic으로 인한 서버 강제 재기동(크래시) 차단 — 예외를 격리하고 Incident로 기록.

| 파일 | 설명 |
| --- | --- |
| `postra-0.10.1-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.10.1`, linux/amd64) |
| `postra-0.10.1-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.10.1-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.10.1-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.10.1-linux-amd64
./postra-0.10.1-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.10.1-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.10.1

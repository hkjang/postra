...
> **v0.18.2**: 발송 상태 화면 추가 — 보낸 메일의 실제 결과(발송됨·재시도 대기·실패·확인 필요)와 서버 응답, 다음 재시도 시각을 확인할 수 있습니다. 일시적 SMTP 오류로 재시도 대기 중인 메일이 그동안 화면에 전혀 보이지 않아 발송된 것으로 오인될 수 있었습니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.18.2-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.18.2`, linux/amd64) |
| `postra-0.18.2-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.18.2-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.18.2-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.18.2-linux-amd64
./postra-0.18.2-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.18.2-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.18.2

...
> **v0.18.0**: AI 분석 결과 화면 개선 — 트리아지·요약·분류·할 일·인물/금액·피싱 점검 결과를 원시 JSON 대신 항목별로 읽기 쉽게 표시합니다. 우선순위·위험 점수는 강조 표시하고, 신뢰도는 백분율로, 할 일은 담당자·기한과 함께 한 줄로 보여줍니다. 원본 응답도 접어서 확인할 수 있습니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.18.0-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.18.0`, linux/amd64) |
| `postra-0.18.0-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.18.0-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.18.0-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.18.0-linux-amd64
./postra-0.18.0-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.18.0-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.18.0

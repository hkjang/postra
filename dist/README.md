...
> **v0.18.1**: 보관·스누즈 실동작 수정 — 보관하거나 미뤄둔 메일이 받은편지함에서 사라지지 않던 문제를 고쳤습니다. 폴더 전환(받은편지함·중요·보관함·스누즈)을 추가하고, 목록에서 기간을 골라 스누즈하거나 보관·스누즈를 해제할 수 있습니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.18.1-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.18.1`, linux/amd64) |
| `postra-0.18.1-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.18.1-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.18.1-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.18.1-linux-amd64
./postra-0.18.1-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.18.1-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.18.1

...
> **v0.9.2**: DB 기반 ObjectStore 단위 테스트 추가 및 워커 Graceful Shutdown 라이프사이클 향상.

| 파일 | 설명 |
| --- | --- |
| `postra-0.9.2-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.9.2`, linux/amd64) |
| `postra-0.9.2-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.9.2-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.9.2-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.9.2-linux-amd64
./postra-0.9.2-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.9.2-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.9.2

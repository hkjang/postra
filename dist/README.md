...
> **v0.10.0**: 시스템 장애/이벤트 모니터링(Incident Dashboard) 웹 UI 및 REST API 추가, 시스템 예외 트래킹 강화.

| 파일 | 설명 |
| --- | --- |
| `postra-0.10.0-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.10.0`, linux/amd64) |
| `postra-0.10.0-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.10.0-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.10.0-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.10.0-linux-amd64
./postra-0.10.0-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.10.0-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.10.0

...
> **v0.10.4**: OIDC 로그인 실패 진단 강화 — Keycloak error_description 노출 및 토큰 교환/검증 실패의 실제 사유(invalid_request 등)를 로그·Incident로 표면화, 저장된 OIDC 설정 공백 trim(redirect_uri 불일치 방지), 인가 요청 파라미터 로깅.

| 파일 | 설명 |
| --- | --- |
| `postra-0.10.4-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.10.4`, linux/amd64) |
| `postra-0.10.4-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.10.4-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.10.4-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.10.4-linux-amd64
./postra-0.10.4-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.10.4-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.10.4

...
> **v0.17.5**: 탐색·접근성 개선 — 상단 메뉴가 현재 위치를 표시하고(메일·대화를 볼 때도 받은편지함으로 유지), 키보드 사용자를 위한 본문 바로가기와 포커스 표시를 추가했으며, 검색 입력·계정 선택에 접근성 라벨을 붙였습니다.

| 파일 | 설명 |
| --- | --- |
| `postra-0.17.5-linux-amd64-image.tar.gz` | `docker load` 로 불러오는 컨테이너 이미지 (`postra:0.17.5`, linux/amd64) |
| `postra-0.17.5-linux-amd64` | 정적 링크 단일 실행 파일 (CGO 없음, 의존성 없음) |
| `postra-0.17.5-sbom.cdx.json` | CycloneDX 소프트웨어 자재명세서 |
...
docker load -i postra-0.17.5-linux-amd64-image.tar.gz     # gzip 자동 인식
...
```bash
chmod +x postra-0.17.5-linux-amd64
./postra-0.17.5-linux-amd64 init
POSTRA_BOOTSTRAP_ADMIN_PASSWORD='replace-me' ./postra-0.17.5-linux-amd64 serve
```
...
go run scripts/mkimage.go postra postra-image.tar postra:0.17.5
